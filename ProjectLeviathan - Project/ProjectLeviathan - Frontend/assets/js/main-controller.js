...
function initMainController() {
    ...
    const handleProfileUpdate = async (button) => {
        ...
        // Se añade la validación de frontend para el nombre de usuario antes de enviarla
        if (field === 'username') {
    ...
    const handlePreferenceUpdate = async (field, value) => {
        ...
        try {
            ...
            const result = await response.json();
            if (result.success) {
                // Actualiza el valor en la configuración local para que no haya que recargar
    ...
    const initializePreferenceControls = () => {
        ...
        document.querySelectorAll('[data-preference-field]').forEach(container => {
            ...
            // Sincronizar interruptores (toggle-switch)
            const toggle = container.querySelector('.toggle-switch input[type="checkbox"]');
            if (toggle) {
                // ========== INICIO DE LA CORRECCIÓN ==========
                // Se cambia la comparación estricta (=== 'true') por una no estricta (== true)
                // para que evalúe correctamente el valor '1' que viene del servidor.
                toggle.checked = (value == true);
                // ========== FIN DE LA CORRECCIÓN ==========
            }

            // Sincronizar selectores (dropdowns)
    ...
    // --- INICIO DE LA NUEVA LÓGICA DE TEMA ---
    ...
    const handleSystemThemeChange = (e) => {
        // Solo aplica el cambio si la preferencia actual del usuario es 'system'
        const currentThemePref = window.PROJECT_CONFIG.userPreferences.theme;
        if (currentThemePref === 'system') {
            applyTheme('system');
        }
    };
    // --- FIN DE LA NUEVA LÓGICA DE TEMA ---
    ...
    function setupEventListeners() {
        ...
        // Se añade el event listener para filtrar la entrada del nombre de usuario en tiempo real.
            ...
            selectorDropdown.querySelectorAll('.menu-link').forEach(link => {
                link.addEventListener('click', () => {
                    ...
                    if (parentItem) {
                        ...
                        // Si el campo cambiado es el tema, aplícalo inmediatamente
        ...
        // Agrega el listener para cambios de tema en el sistema
