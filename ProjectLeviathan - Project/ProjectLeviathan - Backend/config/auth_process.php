...
switch ($action) {
            ...
            try {
                ...
                if ($user && password_verify($password, $user['password'])) {
                    if ($user['status'] !== 'active') {
                        send_response(false, 'Cuenta inactiva.', ['redirect_url' => 'inactive-account.php', 'reason' => $user['status']]);
                        exit;
                    }
                    ...
                    send_response(true, 'Inicio de sesión exitoso.', ['redirect_url' => '../ProjectLeviathan - Frontend/']);
                } else {
    ...
    case 'validate_step1':
        $ip_limit = 3;
        $time_limit_hours = 8; // Límite de tiempo en horas
        ...
        if (!$email) {
            ...
            try {
                if ($user_ip !== 'UNKNOWN') {
                    // Calcular el timestamp de hace 8 horas
                    $time_limit = date('Y-m-d H:i:s', strtotime('-' . $time_limit_hours . ' hours'));

                    // Contar solo las cuentas creadas desde esa IP en las últimas 8 horas
                    $stmt_ip = $pdo->prepare("
                    SELECT COUNT(*) FROM users_metadata 
                    WHERE ip_address = :ip 
                    AND created_at >= :time_limit
                ");
                    $stmt_ip->execute(['ip' => $user_ip, 'time_limit' => $time_limit]);
                    $ip_count = $stmt_ip->fetchColumn();
        ...
        if (!isset($_SESSION['registration_data']) || !isset($_SESSION['client_metadata'])) {
            ...
            try {
                ...
                if (!$verification || new DateTime() > new DateTime($verification['expires_at'])) {
                    ...
                    send_response(true, '¡Cuenta creada!', ['redirect_url' => '../../ProjectLeviathan - Frontend/']);
                }
            } catch (PDOException $e) {
    ...
    case 'reset_password':
        ...
        try {
            ...
            send_response(true, 'Contraseña actualizada.', extra_data: ['redirect_url' => '../']);
        } catch (PDOException $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
